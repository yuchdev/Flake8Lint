...
Documents the exact detection engine used by the `/secret-scan` skill and the
shared scanner implementation (`scripts/secret_scan.py`). When the scanner
source changes, update this file so the catalog stays truthful.
...
> blocked by the very scanner it documents.
...
so loosening it weakens the repository's scanning gate for everyone.
