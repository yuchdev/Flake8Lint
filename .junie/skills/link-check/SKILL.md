...
Note on workflow: in Junie, use `/after-edit` for a scoped post-edit pass and
`/final-verify` before wrapping up larger changes. They rely on the same kinds of
checks, but this skill remains the authoritative on-demand command for outbound
...
1. **Prerequisite — linkify bare mentions** (whole corpus, or scoped to `$ARGUMENTS`):
   ...
   written to `.junie/state/linkify-report.md` for human review.
...
- [ ] Anything `.junie/state/linkify-report.md` could not resolve is surfaced to the user
