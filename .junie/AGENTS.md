...
- Prefer repo-local Junie assets under `.junie/`; treat `.claude/` as legacy source material, not the active home.
- Before ending a substantial coding session, run `/final-verify` so the old Claude stop-hook behavior is still applied explicitly.
- When starting work from a cold context, run `/session-context` to gather the same branch/history/issue briefing the old Claude session-start hook provided.
- Before a risky or unfamiliar shell command, run `/preflight-command <command>` to review it for destructive, network, or protected-path issues.
- After editing files, run `/after-edit <paths>` to apply the old post-edit cleanup flow: secret scan, formatting, link checks, and dependency audit when relevant.
- Use `/github-audit <summary>` after GitHub-facing work when you want an explicit session note replacing the old audit hook.

## Sensitive-path policy imported from Claude settings

- Do not open or read `.env` or `.env.*` files unless the user explicitly asks for it.
- Do not inspect `**/*.dmp` crash dumps or `prototype/*.db` SQLite files unless the user explicitly asks for it.
- Keep shell usage close to the repository's normal workflows: `uv run pytest`, `uv run ruff`, focused `git status/diff/log`, and explicit `gh issue` / `gh pr` work when needed.