...
Skip the gate for a known-WIP session by exporting ``FLAKE8_LINT_SKIP_TESTS=1``.
"""
...
def main() -> None:
    ...
    if event.get("stop_hook_active"):
        allow()
    if os.environ.get("FLAKE8_LINT_SKIP_TESTS") == "1":
        append_log("tests.log", "SKIPPED (FLAKE8_LINT_SKIP_TESTS=1)")
        allow()
    ...
    for label, cmd in steps:
        ...
        if proc.returncode != 0:
            ...
            block(
                f"Stop blocked by Flake8 Linter run-tests: `{' '.join(cmd)}` failed.\n"
                f"{_tail(proc)}\n"
                ...
                "guessing. Export FLAKE8_LINT_SKIP_TESTS=1 only if this is intentional WIP."
            )
