...
- The cross-cutting rules in `@docs/dev/python_coding_standard.md` - full annotations, ruff,
  ...
  never implement their own; and production code must not import from `tests/`. Its
...
- The specific anchor in `docs/architecture.md` or `docs/custom-rules.md` the subtask references -
  the section, not the whole doc. These are the project's design documents; there is no
  `docs/specs/` and no numbered-section convention, so anchors are GitHub heading slugs of the
  `##` headings (`docs/architecture.md#core-rules--registry`, `#config`,
  `#custom-provider-loading`, `#discovery`, `#api-runner`, `#cli`, `#flake8-adapter`,
  `#pytest-helper`; `docs/custom-rules.md#rule-contract`, `#rule-code-naming`,
  `#project-local-registration`, `#installed-provider-packages`, `#-noqa-behavior`). Per
  `CLAUDE.md`, `docs/custom-rules.md` is **required reading** before any subtask adds or changes a
  rule or touches the extension surface - name it explicitly for those subtasks rather than
  relying on the agent to find it.
