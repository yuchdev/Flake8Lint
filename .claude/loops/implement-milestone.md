...
Seed `research_digest.exit_gates` with the four steps `.github/workflows/ci.yml` actually
...
5. The `wheel-smoke` job's script in `.github/workflows/ci.yml`, if the milestone touched CLI
   behavior, config precedence, exit codes, custom rule modules, or entry-point providers. That
   script encodes expected exit codes and output as executable assertions and is the only place
   the installed-wheel path is exercised end to end.

Two conditional gates come from the repo's own conventions (`CLAUDE.md`, "Conventions specific
to this repo"), and R1 should add them when the milestone's tasks match:

- A milestone that adds a **built-in rule** gates on the lockstep set being complete: registry
  entry, `rules.py` implementation, tests, a sample under `tests/samples/`, and README/docs.
- A milestone that changes the **custom-rule extension surface** (`RuleContext`,
  `RuleRegistry`, `RuleViolation`, the registration contract) gates on updated
  `tests/test_custom_rules.py`.

There is **no coverage floor** in this project - `--cov-fail-under` appears nowhere in
`pyproject.toml` or the workflow, so do not invent a percentage. There is **no mandated
security-review gate** either; a `security-auditor` pass is only required when a task is
flagged `security_sensitive` in the cursor, per Step M2. Add nothing beyond the list above.
...
The default above stands: the milestone `status.md` `## Notes & decisions` section is where a
ratified divergence is recorded, and `AskUserQuestion` is the sign-off. This project has no
`docs/adr/` directory yet and names no separate design authority - its ratified conventions live
in `CLAUDE.md` ("Conventions specific to this repo") and `.github/copilot-instructions.md`, both
of which read as already-settled rulings rather than a decision log.

Three additions apply when the divergence is contract-level, because this project has published
contracts that outlive any milestone:

- A ruling that changes **CLI exit codes, config discovery precedence, rule codes, or the
  custom-rule extension surface** must also land a `CHANGELOG.md` entry under the target version
  and, where it contradicts them, an amendment to the matching bullet in `CLAUDE.md` and
  `.github/copilot-instructions.md`. A decision recorded only in `status.md` will be silently
  reverted by the next contributor reading those files.
...
If an ADR directory is later adopted, contract-level rulings from this list should move to
`/adr-write` and step 3 should record the ADR number alongside the `status.md` note.
...
This project's bar is wider than the implement-subtasks default at both levels, and the tests
...
**Milestone complete** = the five gates in the Phase R exit-gate list above (full suite, ruff,
the `--select X001,X009,X010 src` self-check, `python -m build`, and the `wheel-smoke` script
when the milestone touched CLI behavior, config precedence, exit codes, or provider loading),
plus any conditional lockstep gate that the milestone's tasks triggered. There is no staging
deployment; `python -m build` succeeding is the release-readiness bar, and publishing is a
separate workflow (`.github/workflows/publish.yml`), never part of a milestone close.
